...
@router.get(
    "/", response_model=list[UserResponse], dependencies=[Depends(get_manager_or_admin)]
)
...
@router.patch(
    "/{user_id}", response_model=UserResponse, dependencies=[Depends(get_admin)]
)
async def change_user(
    user_id: int, ch_user: UserBase, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(User).where(User.id == user_id))
